...
class MSLog {
	...
	*/
	...
	/**
    * Spaces to put before messages to represent function levels determined by enter and exit
    * @var int
    */
    private $_padLength = 0;
    
    /**
     * Unique identifier to id this session 
     * @var string
     */
	...
	*/
    ...
    /** set $_severityThreshold
     * 
     */
    
    public function setSeverityThreshold( $value ){
    	if ($this->_severityThreshold < $value) {
    		$this->_severityThreshold = intval($value);
    	}
    }
	...
	* @return void
	*/
    public function logDebugSQL($line, $args = self::NO_ARGUMENTS) {
    	$this->log($line, self::DEBUG_SQL);
    }
	...
	*/
     ...
     * @return void
     */
    public function logDebugEnter($line, $args = self::NO_ARGUMENTS) {
    	$this->log($line, self::DEBUG_ENTER);
    	if ($this->_severityThreshold >= self::DEBUG_ENTER) {
    		$this->_padLength++;
    	}
    }
     ...
     * @return void
     */
    public function logDebugExit($line, $args = self::NO_ARGUMENTS) {
    	$this->_padLength--;
    	if ($this->_padLength < 0) {
    		$this->_padLength = 0;
    	}
    	$this->log($line, self::DEBUG_EXIT);
    }
	...
	*/
    public function log($line, $severity, $args = self::NO_ARGUMENTS)
    {
        if ($this->_severityThreshold >= $severity) {
            ...
            //jfds 
            if ($this->_padLength > 0) {
            	$status .= str_pad("", $this->_padLength);
            }
            //jfds $line = "$status $line";
            $line = $status." ".$line;
    ...
    private function _getTimeLine($level)
    {
        ...
        switch ($level) {
            ...
            case self::DEBUG:
                return "$time - DEBUG ----->";
            case self::DEBUG_SQL:
                return "$time - DEBUG_SQL-->";
            case self::DEBUG_ENTER:
            	return "$time - DEBUG_ENTER>";
            case self::DEBUG_EXIT:
            	return "$time - DEBUG_EXIT->";
            case self::ERR:
                return "$time - ERROR-->";
            default:
                return "$time - LOG -->";
